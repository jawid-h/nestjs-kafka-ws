...
export class KafkaClientConfig {
    ...
    public static async fromEnv(env: Record<string, string>) {
        ...
        instance.brokers = (env.KAFKA_BROKERS || '')
            .split(',')
            .map((broker) => broker.trim());
        instance.numberOfRetries = parseInt(
            env.KAFKA_CLIENT_NUMBER_OF_RETRIES || '5',
            10,
        );
        instance.initialRetryTime = parseInt(
            env.KAFKA_CLIENT_INITIAL_RETRY_TIME || '300',
            10,
        );
        ...
        if (validationErrors.length > 0) {
            throw new ConfigValidationError(
                'Kafka client config validation error',
